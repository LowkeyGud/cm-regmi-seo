...
export default function InterpretingSystemLogs() {
        ...
        <pre className="rounded-md bg-black/5 p-4 overflow-x-auto text-sm">
          ...
          <ol className="list-decimal pl-6 text-muted-foreground space-y-2">
            <li>
              ...
              possible) and expand a conservative window (e.g., ±10 minutes) to capture related events.
            </li>
            <li>
              <strong>Filter to noisy sources:</strong> exclude heartbeat or debug noise and focus on
              failing units, kernel errors, and permission denials.
            </li>
            <li>
              <strong>Aggregate repeated lines:</strong> find frequency counts to prioritise the most
              likely root causes instead of chasing one-off traces.
            </li>
            <li>
              <strong>Correlate cross-systems:</strong> align timestamps from web servers, application
              logs, and database nodes to identify a causal chain.
            </li>
            <li>
              <strong>Verify and iterate:</strong> propose a targeted remediation, apply it in a staging
              environment, and rerun the same log filters to confirm the signal disappears.
            </li>
          </ol>
          <h2 className="text-xl font-semibold mt-6">Concrete filtering examples</h2>
          <p className="text-muted-foreground">These examples use <code>journalctl</code> for systemd-managed services.</p>
          <pre className="rounded-md bg-black/5 p-4 overflow-x-auto text-sm">
  ...
  journalctl -u myservice --since "30 minutes ago" -o short-iso | sed -n '1,200p' | grep -n "OutOfMemory" | cut -d: -f1 | xargs -I{} sed -n "$(({}-5)),$(({}+5))p" /tmp/myservice.log`}</code>
          ...
          <p className="text-muted-foreground mb-4">
            ...
            machines can make unrelated events look related. Use ISO timestamps and convert logs to a
            common timezone when comparing multiple hosts.
          </p>
          <pre className="rounded-md bg-black/5 p-4 overflow-x-auto text-sm">
  ...
  # Use ts (from moreutils) to normalise mixed timestamp formats when needed`}</code>
          ...
          <p className="text-muted-foreground mb-4">
            A web service began returning 503s for a subset of requests. Initial application logs showed
            a database connection timeout, but the DB logs showed nothing. After aligning timestamps we
            observed a spike of kernel-level I/O errors on the DB host 30 seconds before the app errors.
            That pointed to an underlying storage issue rather than a connection pool misconfiguration.
          </p>
          <p className="text-muted-foreground mb-4">
            The fix required restoring the offline RAID parity and rerouting traffic to healthy replicas.
            After the repair, re-running the original journalctl filters showed the I/O errors stopped and
            the 503 rate returned to baseline—an explicit verification that the remediation worked.
          </p>
          ...
          <ul className="list-disc pl-6 space-y-2 text-muted-foreground">
            ...
            <li>Capture a short, timestamped evidence bundle (logs + process list + metrics) and attach to the ticket.</li>
            <li>Schedule a follow-up check after 24 hours to ensure no regression appears in logs.</li>
          </ul>
          <h2 className="text-xl font-semibold mt-6">Avoiding common mistakes</h2>
          <p className="text-muted-foreground mb-4">
            Don’t fix what you can’t reproduce. Avoid over-indexing on a single dramatic message without
            frequency counts, and never remove or alter logs before archiving a copy for forensic review.
          </p>
...
journalctl -u myservice --since "2026-05-24" | grep -i error | sort | uniq -c | sort -rn`}</code>
      ...
      <section className="space-y-6 mx-auto max-w-3xl px-6 pb-12">
        ...
        <p className="text-muted-foreground leading-relaxed">
          For complex systems, use structured logging and a correlation ID propagated across services.
          This makes it trivial to gather all events for a request and reason about causality. When
          structured logs are unavailable, create a small envelope that maps timestamps to request IDs
          so you can join logs from multiple services deterministically.
        </p>
        <p className="text-muted-foreground leading-relaxed">
          Tools like the ELK stack, Loki, or a hosted APM can index logs and provide fast correlation
          queries; however, the same principles apply: anchor the timeframe, filter noise, and focus
          on repeated, actionable signals rather than one-off traces.
        </p>
        <p className="text-muted-foreground leading-relaxed">
          Maintain a short runbook for common log patterns (OOMs, connection resets, permission denied)
          with example queries and expected verification steps so responders can act quickly.
        </p>
